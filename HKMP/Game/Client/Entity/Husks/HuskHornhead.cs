...
{
    // HuskHornhead has all the fields/properties/methods of HealthManagedEntity
    public class HuskHornhead : HealthManagedEntity
    {
        // Private fields created for HuskHornhead
        ...
        ) : base(netClient, EntityType.HuskHornhead, entityId, gameObject)
        {
            // Initializes fields for HuskHornhead instance
        ...
        {
            Logger.Get().Info(this, "HuskHornhead: Switching to Scene Host");
        ...
        public override void UpdateAnimation(byte animationIndex, byte[] animationInfo)
        {
            ...
            if (animation == Animation.Anticipate)
            {
                ...
                //Logger.Get().Info(this, "HuskHornhead Executing Anticipate");
            }

            if (animation == Animation.Lunge)
            {
                _fsm.ExecuteActions("Lunge", 1, 2);
                Logger.Get().Info(this, "HuskHornhead Executing Lunge");
            }

            if (animation == Animation.Cooldown)
            {
                _fsm.ExecuteActions("Cooldown", 1, 2);
                //Logger.Get().Info(this, "HuskHornhead Executing Cooldown");
            }
            ...
            if (animation == Animation.Idle)
            {
                ...
                _audioSource.Stop();
                //Logger.Get().Info(this, "HuskHornhead Playing Idle");
            }
            if (animation == Animation.Walk)
            {
                _audioSource.Play();
                _walker.GetComponent<tk2dSpriteAnimator>().Play("Walk");
                //Logger.Get().Info(this, "HuskHornhead playing Walk");
            }
