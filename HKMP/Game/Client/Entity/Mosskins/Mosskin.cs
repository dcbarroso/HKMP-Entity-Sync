...
{
    // Mosskin has all the fields/properties/methods of HealthManagedEntity
    public class Mosskin : HealthManagedEntity
    {
        // Private fields created for Mosskin
        ...
        ) : base(netClient, EntityType.Mosskin, entityId, gameObject)
        {
            // Initializes variables for Mosskin instance
        ...
        {
            Logger.Get().Info(this, "Mosskin: Switching to Scene Host");
        ...
        public override void UpdateAnimation(byte animationIndex, byte[] animationInfo)
        {
            ...
            {
                _fsm.ExecuteActions("Anticipate", 2, 3, 6);
            }

            if (animation == Animation.Lunge)
            {
                _fsm.ExecuteActions("Lunge", 1, 4, 5);
            }

            if (animation == Animation.Cooldown)
            {
                _fsm.ExecuteActions("Cooldown", 1);
            }
